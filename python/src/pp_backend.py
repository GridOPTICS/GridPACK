...
class EducPandaPowerBackend(Backend):
    ...
    def load_grid(self,
        ...
        try:
            with warnings.catch_warnings():
                ...
                # print(self._grid.res_load, self._grid.res_gen, self._grid.res_line, self._grid.res_trafo)
                # sys.exit(1)
        ...
        print(self._grid.line, self._grid.res_line, self._grid.res_gen, self._grid.res_bus)
        
        pp.runpp(self._grid, check_connectivity=False)
        ######################################################################

        # print(self._grid.gen[["p_mw"]])
        # print(self._grid.load[["p_mw"]])
        # print(self._grid.res_line[["p_from_mw", "q_from_mvar", "p_to_mw", "q_to_mvar"]])
        # print(self._grid.res_gen[["p_mw", "q_mvar"]])
        # print(self._grid.res_load[["p_mw", "q_mvar"]])
        # sys.exit(1)
    ...
    def apply_action(self, backendAction: Union["grid2op.Action._backendAction._BackendAction", None]) -> None:
        ...
        for gen_id, new_p in prod_p:
            self._grid.gen.iloc[gen_id]["p_mw"] = new_p
        for gen_id, new_v in prod_v:
            self._grid.gen.iloc[gen_id]["vm_pu"] = new_v  # but new_v is not pu !
            self._grid.gen.iloc[gen_id]["vm_pu"] /= self._grid.bus["vn_kv"][
                self.gen_to_subid[gen_id]
            ]  # now it is :-)

        for load_id, new_p in load_p:
            self._grid.load.iloc[load_id]["p_mw"] = new_p
        for load_id, new_q in load_q:
            self._grid.load.iloc[load_id]["q_mvar"] = new_q
    ...
    def generators_info(self)-> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        prod_v *= (
            self._grid.bus.iloc[self.gen_to_subid]["vn_kv"].values.astype(dt_float)
        )  # in kV
    ...
    def lines_or_info(self)-> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        # print(f"Res line: {self._grid.res_line} \n Line: {self._grid.line} \n Trafo: {self._grid.res_trafo}")
        # sys.exit(1)
    ...
    def _disconnect_line(self, id_ : int) -> None:
        ...
        if id_ < self._nb_real_line_pandapower:
            self._grid.line.iloc[id_]["in_service"] = False
        else:
            self._grid.trafo.iloc[
    ...
    def reset(self,
        ...
        self._grid.line.loc[:, ["from_bus"]] = self.line_or_to_subid[
            : self._nb_real_line_pandapower
        ]
        self._grid.trafo.loc[:, ["hv_bus"]] = self.line_or_to_subid[
            self._nb_real_line_pandapower :
        ]
        self._grid.line.loc[:, ["to_bus"]] = self.line_ex_to_subid[
            : self._nb_real_line_pandapower
        ]
        self._grid.trafo.loc[:, ["lv_bus"]] = self.line_ex_to_subid[
            self._nb_real_line_pandapower :
        ]
        self._grid.load.loc[:, ["bus"]] = self.load_to_subid
        self._grid.gen.loc[:, ["bus"]] = self.gen_to_subid

        # originally everything is in service
        self._grid.line.loc[:, ["in_service"]] = True
        self._grid.trafo.loc[:, ["in_service"]] = True
        self._grid.load.loc[:, ["in_service"]] = True
        self._grid.gen.loc[:, ["in_service"]] = True

        self._grid.bus.iloc[: self.n_sub]["in_service"] = True
        self._grid.bus.iloc[self.n_sub :]["in_service"] = False
