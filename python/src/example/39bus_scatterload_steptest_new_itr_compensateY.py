...
while (not hadapp.isDynSimuDone()):
    ...
    if isteps == int(5.0/simu_time_step):
        ...
        hadapp.scatterInjectionLoadNew_compensateY(buslist, plist, qlist)
    ...
    if ((isteps >= int(45.0/simu_time_step)) and (isteps%nstep_change == 0)):
        ...
        qlist = [qtmp, qtmp1]
        # the scatterInjectionLoad will change the loads at the buslist to be the values in the plist and qlist
        hadapp.scatterInjectionLoadNew_compensateY(buslist, plist, qlist) # note the buslist, plist, qlist can be lists, which means you can change multiple loads at different buses
...
csv_wrt_f = inname[6:-4] + '_loadchange_new_compensateY_eachstep'
