...
This document provides a step-by-step guide to install GridPACK and its dependencies on EIOC-Ubuntu8. They are meant to be used with GridPACK's branch `feature/three-sequence` which has updates for compatibility with PETSc's latest release 3.16.4.
...
```
Step 2. Change branch to the feature branch
```
