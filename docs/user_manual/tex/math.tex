...
reached or \textbf{\texttt{maxsteps}} time steps are completed.  A
subsequent call to \textbf{\texttt{solve}} will continue where the
...
the evolution \texttt{\textbf{DAESolver}} time integration.  These
functions take the current time as the only argument.  To illustrate
ways to use these, \textbf{\texttt{MyDAEHelper}} might be modified to
add some facilities and pass those to the solver: 
