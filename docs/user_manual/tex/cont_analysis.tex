...
The first few lines in the \texttt{\textbf{getContingencies}} function are used
...
A \texttt{\textbf{CursorPtr}} is defined and set to point to the contents of the
...
Note that it has two blocks, \texttt{\textbf{Contingency\_analysis}} and
...
are used to print out a list of how many tasks were evaluated on each processor and to stop the timing of the ``\texttt{\textbf{Total Application}}'' category. Results can optionally be exported to JSON or CSV by setting \texttt{\textbf{outputFormat}} and \texttt{\textbf{outputFile}} in the \texttt{\textbf{Contingency\_analysis}} block (see the power flow module documentation for output format details). The timer \texttt{\textbf{dump}} method will print statistics on the amount of time spent in the total application as well as reporting timings inside the power flow application. The check on the \texttt{\textbf{dump}} call is to verify that all processors have participated in at least one power flow calculation.
