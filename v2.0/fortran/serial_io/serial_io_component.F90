...
!
  logical function bus_serial_write(bus, string, bufsize, signal)
    ...
    write(string,'(a,i4,a,i4)') '  Bus: ',bus%bus_get_original_index(), &
       '      ',bus%bus_get_global_index()
...
!
  logical function branch_serial_write(branch, string, bufsize, signal)
    ...
    integer, value, intent(in) :: bufsize
    character(len=*), intent(in) :: signal
    write(string,'(a,i4,a,i4,a,i4,a,i4)') '  Branch: ', &
      branch%branch_get_bus1_original_index(),'      ', &
      branch%branch_get_bus2_original_index(),'      ', &
      branch%branch_get_bus1_global_index(),'      ', &
      branch%branch_get_bus2_global_index()
