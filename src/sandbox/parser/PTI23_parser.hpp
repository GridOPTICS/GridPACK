  ...
  //      std::vector<gridpack::component::DataCollection>   case_instance;
      ...
      void find_buses(std::ifstream & input)
      {
        ...
        while(test_end(line)) {
          ...
          // BUS_I               "I"                   integer
          // o_idx is original from file
          o_idx = atoi(split_line[0].c_str());
          data->addValue(BUS_NUMBER, o_idx);
          p_busData.push_back(data);

          // index is the sequential index of the object
 ...
 //         p_busData[o_idx]->addValue(GENERATOR_XT, atof(split_line[12].c_str()));
      ...
      void find_branches(std::ifstream & input)
      {
        ...
        while(test_end(line)) {
          ...
          // this is the gridpack index, not the pti index
          ...
          p_branchMap.insert(std::pair<stp::pair<int, int>, int >(branch_pair, index));
...
//          data->addValue(BRANCH_SHUNT_ADMTTNC_B1, atof(split_line[10].c_str()));
      ...
      void find_transformer(std::ifstream & input)
      {
        ...
        while(test_end(line)) {
          ...
          data->addValue(BRANCH_FROMBUS, fromBus);
          ...
          // KG: I'm assuming the BRANCH_FROMBUS is the bus index we need to match
          int toBus = atoi(split_line[0].c_str());
          data->addValue(BRANCH_TOBUS, toBus);
...
//          data->addValue(TRANSFORMER_BUS3, atoi(split_line[1].c_str()));
           ...
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_CKT, split_line[2].c_str());

          /*
           * type: integer
           * #define TRANSFORMER_CW "TRANSFORMER_CW"
           X            */
          p_branchData[o_idx]->addValue(TRANSFORMER_CW, atoi(split_line[3].c_str()));

          /*
           * type: integer
           * #define TRANSFORMER_CZ "TRANSFORMER_CZ"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_CZ, atoi(split_line[5].c_str()));

          /*
           * type: integer
           * #define TRANSFORMER_CM "TRANSFORMER_CM"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_CM, atoi(split_line[5].c_str()));

          /*
           * type: real float
           * #define TRANSFORMER_MAG1 "TRANSFORMER_MAG1"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_MAG1, atof(split_line[5].c_str()));

          /*
           * type: real float
           * #define TRANSFORMER_MAG2 "TRANSFORMER_MAG2"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_MAG2, atof(split_line[5].c_str()));

          /*
           * type: integer
           * #define TRANSFORMER_NMETR "TRANSFORMER_NMETR"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_NMETR, atoi(split_line[1].c_str()));

          /*
           * type: string
           * #define TRANSFORMER_NAME "TRANSFORMER_NAME"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_NAME, split_line[2].c_str());
           ...
           *
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_STATUS, atoi(split_line[1].c_str()));

          /*
           * type: integer
           * #define TRANSFORMER_OWNER "TRANSFORMER_OWNER"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_OWNER, atoi(split_line[1].c_str()));

          /*
           * type: real float
           * #define TRANSFORMER_R1_2 "TRANSFORMER_R1_2"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_R1_2, atof(split_line[1].c_str()));

          /*
           * type: real float
           * #define TRANSFORMER_X1_2 "TRANSFORMER_X1_2"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_X1_2, atof(split_line[1].c_str()));

          /*
           * type: real float
           * #define TRANSFORMER_SBASE1_2 "TRANSFORMER_SBASE1_2"
           */
          p_branchData[o_idx]->addValue(TRANSFORMER_SBASE1_2, atof(split_line[1].c_str()));
