...
#ifndef BASEMATRIXMAP_HPP_
#define BASEMATRIXMAP_HPP_
 ...
 */
BaseMatrixMap(boost::shared_ptr<_network> network)
  : p_network(network)
{
  p_i_busOffsets = NULL;
  p_j_busOffsets = NULL;
 ...
 */
boost::shared_ptr<gridpack::math::Matrix> baseMapToMatrix(bool isDense = false)
{
 ...
 * @return return a pointer to new matrix
 */
boost::shared_ptr<gridpack::math::RealMatrix> baseMapToRealMatrix(
    bool isDense = false)
{
  gridpack::parallel::Communicator comm = p_network->communicator();
  int t_new, t_bus, t_branch, t_set;
 ...
 */
void baseMapToMatrix(gridpack::math::Matrix &matrix)
{
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseMapToRealMatrix(gridpack::math::RealMatrix &matrix)
{
  int t_set, t_bus, t_branch;
  GA_Pgroup_sync(p_GAgrp);
  if (p_timer) t_set = p_timer->createCategory("Mapper: Set Matrix");
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseMapToMatrix(boost::shared_ptr<gridpack::math::Matrix> &matrix)
{
  baseMapToMatrix(*matrix);
}
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseMapToRealMatrix(boost::shared_ptr<gridpack::math::RealMatrix> &matrix)
{
  baseMapToRealMatrix(*matrix);
}
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseOverwriteMatrix(gridpack::math::Matrix &matrix)
{
  GA_Pgroup_sync(p_GAgrp);
  loadBusData(matrix,false);
  loadBranchData(matrix,false);
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseOverwriteMatrix(boost::shared_ptr<gridpack::math::Matrix> &matrix)
{
  baseOverwriteMatrix(*matrix);
}
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseIncrementMatrix(gridpack::math::Matrix &matrix)
{
  GA_Pgroup_sync(p_GAgrp);
 ...
 * @param matrix existing matrix (should be generated from same mapper)
 */
void baseIncrementMatrix(boost::shared_ptr<gridpack::math::Matrix> &matrix)
{
  baseIncrementMatrix(*matrix);
}
...
bool baseCheck(void)
{
  ...
  for (i=0; i<p_nBranches; i++) {
    ...
    isize = 0;
    jsize = 0;
    if (p_network->getBranch(i)->baseMatrixForwardSize(&isize,&jsize)) {
      p_network->getBranch(i)->baseGetMatVecIndices(&idx, &jdx);
      if (idx >= p_minRowIndex && idx <= p_maxRowIndex) {
        ok = true;
        msize = 0;
        nsize = 0;
        bus1->baseMatrixDiagSize(&msize, &nsize);
        if (nsize != isize) ok = false;
        msize = 0;
        nsize = 0;
        bus2->baseMatrixDiagSize(&msize, &nsize);
    ...
    isize = 0;
    jsize = 0;
    if (p_network->getBranch(i)->baseMatrixReverseSize(&isize,&jsize)) {
      p_network->getBranch(i)->baseGetMatVecIndices(&idx, &jdx);
...
void createIndexGA(int * handle, int size)
{
  ...
  *handle = GA_Create_handle();
  ...
  if (!GA_Allocate(*handle)) {
    char buf[256];
    sprintf(buf,"BaseMatrixMap::createIndexGA: Unable to allocate distributed array\n");
    printf("%s",buf);
        ...
        int ** iIndexArray, int ** jIndexArray, int *icount, int *jcount)
{
  ...
  *jcount = 0;
  ...
  for (i = 0; i < p_nBuses; i++) {
//    jSize = 0;
    status = p_network->getBus(i)->baseMatrixDiagSize(&iSize, &jSize);
    int isave = iSize;
    if (status) {
      maxcol = 0;
      p_network->getBus(i)->baseGetMatVecIndex(&index);
      if (iSize > 0) {
...
//      bool addrow = false;
      ...
      for(j = 0; j<branches.size(); j++) {
        branches[j]->baseGetMatVecIndices(&idx, &jdx);
        if (index == idx) {
          chk = branches[j]->baseMatrixForwardSize(&iSize, &jSize);
        } else {
          chk = branches[j]->baseMatrixReverseSize(&iSize, &jSize);
        }
        if (chk) maxcol += jSize;
#ifdef NZ_PER_ROW
//        addrow = addrow || chk;
        ...
        int * jcount)
{
  ...
  *jcount = 0;
  for (int i = 0; i < p_nBranches; i++) {
    status = p_network->getBranch(i)->baseMatrixForwardSize(&iSize, &jSize);
    if (status) {
      p_network->getBranch(i)->baseGetMatVecIndices(&iIndex, &jIndex);
      if (iSize > 0) {
        iSizeArray[*icount]      = iSize;
        ...
        int * jcount)
{
  ...
  *jcount = 0;
  for (int i = 0; i < p_nBranches; i++) {
    status = p_network->getBranch(i)->baseMatrixReverseSize(&iSize, &jSize);
    if (status) {
      p_network->getBranch(i)->baseGetMatVecIndices(&iIndex, &jIndex);
      if (iSize > 0) {
        iSizeArray[*icount]      = iSize;
...
//    }
  ...
  itmp[p_me] = iSize;
  jtmp[p_me] = jSize;
//  printf("p[%d] (BaseMatrixMap) iSize: %d jSize: %d\n",p_me,iSize,jSize);
...
//  printf("p[%d] (BaseMatrixMap) activeBuses: %d\n",p_me,p_activeBuses);
  ...
  if (!GA_Allocate(gaOffsetI)) {
    char buf[256];
    sprintf(buf,"BaseMatrixMap::setupOffsetArrays: Unable to allocate distributed array for row offsets\n");
    printf("%s",buf);
    throw gridpack::Exception(buf);
  }
  GA_Zero(gaOffsetI);
  ...
  if (!GA_Allocate(gaOffsetJ)) {
    char buf[256];
    sprintf(buf,"BaseMatrixMap::setupOffsetArrays: Unable to allocate distributed array for column offsets\n");
    printf("%s",buf);
    throw gridpack::Exception(buf);
  }
  GA_Zero(gaOffsetJ);
...
void setBusOffsets(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      ...
      if (bus->baseMatrixDiagSize(&isize,&jsize)) {
        indices[icnt] = ptr;
        bus->baseGetMatVecIndex(&idx);
        *(indices[icnt]) = idx;
        ptr++;
        icnt++;
      }
  ...
  if (icnt != p_busContribution) {
    char buf[256];
    sprintf(buf,"BaseMatrixMap::setBusOffsets: Contribution from buses doesn't match\n");
    printf("%s",buf);
    throw gridpack::Exception(buf);
  }
...
#endif
        if (bus->baseMatrixDiagValues(values)) {
          icnt = 0;
          for (k=0; k<jsize; k++) {
...
void setBranchOffsets(void)
{
  ...
  for (i=0; i<p_nBranches; i++) {
    ...
    if (branch->baseMatrixForwardSize(&isize,&jsize)) {
      branch->baseGetMatVecIndices(&idx, &jdx);
      if (idx >= p_minRowIndex && idx <= p_maxRowIndex) {
    ...
    if (branch->baseMatrixReverseSize(&isize,&jsize)) {
      branch->baseGetMatVecIndices(&idx, &jdx);
      if (jdx >= p_minRowIndex && jdx <= p_maxRowIndex) {
        i_indices[icnt] = i_ptr;
        j_indices[icnt] = j_ptr;
  ...
  for (i=0; i<p_nBranches; i++) {
    ...
    if (branch->baseMatrixForwardSize(&isize,&jsize)) {
      ...
      if (idx >= p_minRowIndex && idx <= p_maxRowIndex) {
        ...
        for (k=0; k<ijsize; k++) values[k] = 0.0;
#endif
        if (branch->baseMatrixForwardValues(values)) {
          icnt = 0;
          for (k=0; k<jsize; k++) {
    ...
    if (branch->baseMatrixReverseSize(&isize,&jsize)) {
      ...
      if (jdx >= p_minRowIndex && jdx <= p_maxRowIndex) {
        ...
        for (k=0; k<ijsize; k++) values[k] = 0.0;
#endif
        if (branch->baseMatrixReverseValues(values)) {
          icnt = 0;
...
void contributions(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      if (p_network->getBus(i)->baseMatrixDiagSize(&isize, &jsize))
        p_busContribution++;
    }
  }
