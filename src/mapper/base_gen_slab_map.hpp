...
#ifndef BASEGENSLABMAP_HPP_
#define BASEGENSLABMAP_HPP_
 ...
 */
BaseGenSlabMap(boost::shared_ptr<_network> network)
  : p_network(network)
{
  p_Offsets = NULL;
 ...
 * created with the mapToMatrix method using the same BaseGenSlabMap
 * @param matrix matrix containing data to be pushed to network
 */
void baseMapToNetwork(const gridpack::math::Matrix &matrix)
{
#if 1
  int i, j, k;
  ComplexType *values;
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      p_network->getBus(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBus(i)->baseSlabGetRowIndices(iptr);
      iptr += nrows;
    }
  }
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      p_network->getBus(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBus(i)->baseSlabSetValues(vptr);
      vptr += nrows;
    }
  }
  delete [] idx;
  ...
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      p_network->getBranch(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBranch(i)->baseSlabGetRowIndices(iptr);
      iptr += nrows;
    }
  }
  ...
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      p_network->getBranch(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBranch(i)->baseSlabSetValues(vptr);
      vptr += nrows;
    }
  }
  delete [] idx;
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      p_network->getBus(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBus(i)->baseSlabGetRowIndices(idx);
      for (j=0; j<nrows; j++) {
        for (k=0; k<ncols; k++) {
          matrix.getElement(idx[j], k, (values[j])[k]);
        }
      }
      p_network->getBus(i)->baseSlabSetValues(values);
    }
  }
  // get values from branches
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      p_network->getBranch(i)->baseSlabSize(&nrows,&ncols);
      p_network->getBranch(i)->baseSlabGetRowIndices(idx);
      for (j=0; j<nrows; j++) {
        for (k=0; k<ncols; k++) {
          matrix.getElement(idx[j], k, (values[j])[k]);
        }
      }
      p_network->getBranch(i)->baseSlabSetValues(values);
    }
  }
  for (i=0; i<p_maxValues; i++) {
    delete[] values[i];
 ...
 * @param matrix matrix containing data to be pushed to network
 */
void baseMapToNetwork(boost::shared_ptr<gridpack::math::Matrix> &matrix)
{
  baseMapToNetwork(*matrix);
}
...
void getDimensions(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      p_network->getBus(i)->baseSlabSize(&ival,&jval);
  ...
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      p_network->getBranch(i)->baseSlabSize(&ival,&jval);
      if (p_maxValues < ival) p_maxValues = ival;
      nRows += ival;
      p_branchRows += ival;
      if (nCols == 0 && ival > 0) nCols = jval;
  ...
  if (!okCols && p_me == 0) {
    char buf[512];
    sprintf(buf,"(BaseGenSlabMap) Number of columns not uniform across all processors\n");
...
void setOffsets(void)
{
      ...
      for (j=0; j<nsize; j++) {
        ...
        if (isLocalBranch(jdx)) {
          ...
          if (jdx1 == i) {
            ...
            p_network->getBranch(jdx)->baseSlabSize(&ival,&jval);
            icnt += ival;
          }
        } else {
  ...
  if (!GA_Allocate(g_bus_offsets)) {
    char buf[256];
    sprintf(buf,"BaseGenSlabMap::setOffsets: Unable to allocate distributed array for bus offsets\n");
    printf("%s",buf);
    throw gridpack::Exception(buf);
  }
  ...
  if (!GA_Allocate(g_branch_offsets)) {
    char buf[256];
    sprintf(buf,"BaseGenSlabMap::setOffsets: Unable to allocate distributed array for branch offsets\n");
    printf("%s",buf);
    throw gridpack::Exception(buf);
  }
...
void setIndices(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    ...
    if (nrows > 0) {
      ...
      for (j=0; j<nrows; j++) {
        idx = offset+j;
        p_network->getBus(i)->baseSlabSetRowIndex(j,idx);
      }
    }
  }
  for (i=0; i<p_nBranches; i++) {
    ...
    if (nrows > 0) {
      ...
      for (j=0; j<nrows; j++) {
        idx = offset+j;
        p_network->getBranch(i)->baseSlabSetRowIndex(j,idx);
      }
    }
  }
...
void loadBusData(gridpack::math::Matrix &matrix, bool flag)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      p_network->getBus(i)->baseSlabSize(&ivals,&jvals);
      p_network->getBus(i)->baseSlabGetValues(values, idx);
      for (j=0; j<ivals; j++) {
        if (flag) {
...
void loadBranchData(gridpack::math::Matrix &matrix, bool flag)
{
  ...
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      p_network->getBranch(i)->baseSlabSize(&ivals,&jvals);
      p_network->getBranch(i)->baseSlabGetValues(values,idx);
      for (j=0; j<ivals; j++) {
