...
#ifndef BASEVECTORMAP_HPP_
#define BASEVECTORMAP_HPP_
 ...
 */
BaseVectorMap(boost::shared_ptr<_network> network)
  : p_network(network)
{
  p_Offsets                        = NULL;
  p_ISize                          = NULL;
 ...
 * @return return pointer to new vector
 */
boost::shared_ptr<gridpack::math::RealVector> baseMapToRealVector(void)
{
  gridpack::parallel::Communicator comm = p_network->communicator();
  int t_new(0), t_bus(0), t_set(0);
  if (p_timer) t_new = p_timer->createCategory("Vector Map: New Vector");
 ...
 * @param vector existing vector that should be reset
 */
void baseMapToRealVector(gridpack::math::RealVector &vector)
{
  int t_bus(0), t_set(0);
  if (p_timer) t_set = p_timer->createCategory("Vector Map: Set Vector");
  if (p_timer) p_timer->start(t_set);
 ...
 * @param vector existing vector that should be reset
 */
void baseMapToRealVector(boost::shared_ptr<gridpack::math::RealVector> &vector)
{
  baseMapToRealVector(*vector);
}

/**
 * Push data from vector onto buses. Vector must be created with the
 * mapToVector method using the same BaseVectorMap
 * @param vector vector containing data to be pushed to buses
 */
void baseMapToBus(const gridpack::math::Vector &vector)
{
  ...
  for (i=0; i<p_busContribution; i++) {
    p_contributingBuses[i]->baseSetValues(vptr);
    isize = p_ISize[i];
    vptr += isize;
  }
 ...
 * @param vector vector containing data to be pushed to buses
 */
void baseMapToBus(const gridpack::math::RealVector &vector)
{
 ...
 * @param vector vector containing data to be pushed to buses
 */
void baseMapToBus(boost::shared_ptr<gridpack::math::Vector> &vector)
{
  baseMapToBus(*vector);
}

void baseMapToBus(boost::shared_ptr<gridpack::math::RealVector> &vector)
{
  baseMapToBus(*vector);
}
...
void loadBusData(gridpack::math::Vector &vector, bool flag)
{
  ...
  icnt = 0;
  for (i=0; i<p_busContribution; i++) {
    p_contributingBuses[i]->baseVectorValues(vptr);
    isize = p_ISize[i];
    idx = p_Offsets[i];
    for (j=0; j<isize; j++) {
      ibuf[icnt] = idx;
...
void contributions(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      if (p_network->getBus(i)->baseVectorSize(&isize)) {
        p_busContribution++;
        p_numValues += isize;
      }
    }
