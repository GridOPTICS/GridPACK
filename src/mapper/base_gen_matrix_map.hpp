...
#ifndef BASEGENMATRIXMAP_HPP_
#define BASEGENMATRIXMAP_HPP_
 ...
 */
BaseGenMatrixMap(boost::shared_ptr<_network> network)
  : p_network(network)
{
  p_row_Offsets = NULL;
  p_col_Offsets = NULL;
 ...
 * function is called
 * @param mode enumerated parameter describing what interface should be
 used 
 */
 ...
 */
boost::shared_ptr<gridpack::math::Matrix> baseMapToMatrix(void)
{
...
void getDimensions(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      nval = p_network->getBus(i)->baseMatrixNumRows();
      nRows += nval;
      nval = p_network->getBus(i)->baseMatrixNumCols();
      nCols += nval;
    }
  }
  ...
  for (i=0; i<p_nBranches; i++) {
    if (p_network->getActiveBranch(i)) {
      nval = p_network->getBranch(i)->baseMatrixNumRows();
      nRows += nval;
      nval = p_network->getBranch(i)->baseMatrixNumCols();
      nCols += nval;
    }
  }
...
void setOffsets(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      ...
      icnt += p_network->getBus(i)->baseMatrixNumRows();
      j_bus_offsets[i] = jcnt;
      jcnt += p_network->getBus(i)->baseMatrixNumCols();
      ...
      for (j=0; j<nsize; j++) {
        ...
        if (isLocalBranch(jdx)) {
          ...
          if (jdx1 == i) {
            ...
            icnt += p_network->getBranch(jdx)->baseMatrixNumRows();
            j_branch_offsets[jdx] = jcnt;
            jcnt += p_network->getBranch(jdx)->baseMatrixNumCols();
          }
        } else {
...
void setIndices(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    nrows = p_network->getBus(i)->baseMatrixNumRows();
    if (nrows > 0) {
      offset = i_bus_value_buf[i];
      for (j=0; j<nrows; j++) {
        idx = offset+j;
        p_network->getBus(i)->baseMatrixSetRowIndex(j,idx);
      }
    }
    ncols = p_network->getBus(i)->baseMatrixNumCols();
    if (ncols > 0) {
      offset = j_bus_value_buf[i];
      for (j=0; j<ncols; j++) {
        idx = offset+j;
        p_network->getBus(i)->baseMatrixSetColIndex(j,idx);
      }
    }
  }
  for (i=0; i<p_nBranches; i++) {
    nrows = p_network->getBranch(i)->baseMatrixNumRows();
    if (nrows > 0) {
      offset = i_branch_value_buf[i];
      for (j=0; j<nrows; j++) {
        idx = offset+j;
        p_network->getBranch(i)->baseMatrixSetRowIndex(j,idx);
      }
    }
    ncols = p_network->getBranch(i)->baseMatrixNumCols();
    if (ncols > 0) {
      offset = j_branch_value_buf[i];
      for (j=0; j<ncols; j++) {
        idx = offset+j;
        p_network->getBranch(i)->baseMatrixSetColIndex(j,idx);
      }
    }
  }
...
void numberNonZeros(void)
{
  ...
  for (i=0; i<p_nBuses; i++) {
    if (p_network->getActiveBus(i)) {
      nvals = p_network->getBus(i)->baseMatrixNumValues();
      ...
      if (nvals > 0) {
        ...
        p_network->getBus(i)->baseMatrixGetValues(values, rows, cols);
        for (j=0; j<nvals; j++) {
...
//          }
  ...
  for (i=0; i<p_nBranches; i++) {
    nvals = p_network->getBranch(i)->baseMatrixNumValues();
    if (nvals > p_maxValues) p_maxValues = nvals;
    if (nvals > 0) {
      int ncols = p_network->getBranch(i)->baseMatrixNumCols();
      int rmin, rmax;
      bool isActive = p_network->getActiveBranch(i);
      if (ncols > 0) {
        rmin = p_network->getBranch(i)->baseMatrixGetRowIndex(0);
        rmax = p_network->getBranch(i)->baseMatrixGetRowIndex(ncols-1);
      }
      gridpack::ComplexType *values = new gridpack::ComplexType[nvals];
      int *rows = new int[nvals];
      int *cols = new int[nvals];
      p_network->getBranch(i)->baseMatrixGetValues(values, rows, cols);
      for (j=0; j<nvals; j++) {
        if (rows[j] >= p_minRowIndex && rows[j] <= p_maxRowIndex) {
          if (ncols > 0) {
