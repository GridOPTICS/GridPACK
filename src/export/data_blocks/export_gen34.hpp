 ...
 * export_gen34.hpp
 *
 * This class exports generator data in PSS/E v34 format
 *
 *  Created on: October 19, 2023
 *      Author: Bruce Palmer
 */

#ifndef EXPORTGEN34_HPP_
#define EXPORTGEN34_HPP_
...
class ExportGen34
{
  public:
     ...
     * Constructor
     */
    explicit ExportGen34(boost::shared_ptr<_network> network) :
      ...
      for (i=0; i<nbus; i++) {
        if (p_network->getActiveBus(i)) {
          ...
          for (j=0; j<ngen; j++) {
            ...
            ptr += strlen(ptr);
            ival = 0;
            data->getValue(GENERATOR_NREG,&ival,j);
            sprintf(ptr," %d\n",ival);
            text_line text;
