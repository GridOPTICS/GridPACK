 ...
 * export_bus34.hpp
 *
 * This class exports bus data in PSS/E v34 format
 *
 *  Created on: October 19, 2023
 *      Author: Bruce Palmer
 */

#ifndef EXPORTBUS34_HPP_
#define EXPORTBUS34_HPP_
...
class ExportBus34
{
  public:
     ...
     * Constructor
     */
    explicit ExportBus34(boost::shared_ptr<_network> network) :
    ...
    void writeBusBlock(std::ofstream &fout)
    {
      ...
      if (me == 0) {
        fout << case_id<<",  "<<sbase<<",  34, 0, 0, 60.0"<<std::endl;
        fout << std::endl;
        fout << "/ BEGIN SYSTEM-WIDE DATA" << std::endl;
        fout << "0 / END SYSTEM-WIDE DATA, BEGIN BUS DATA" << std::endl;
      }
