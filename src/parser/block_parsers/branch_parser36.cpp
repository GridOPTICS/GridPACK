 ...
 * @param branch_map map bus index pair in RAW file to
 internal indices
 */
gridpack::parser::BranchParser36::BranchParser36(
 ...
 * Simple Destructor
 */
gridpack::parser::BranchParser36::~BranchParser36(void)
{
}
 ...
 */
void gridpack::parser::BranchParser36::parse(
    ...
    std::vector<boost::shared_ptr<gridpack::component::DataCollection> > &p_branchData)
{
  ...
  while(test_end(line)) {
    ...
    // BRANCH_BYPASS         "BP"                  integer
    if (nstr > 24) p_branchData[l_idx]->addValue(BRANCH_BYPASS,
        atoi(split_line[24].c_str()), nelems);

    // BRANCH_METER         "MET"                  integer
    if (nstr > 25) p_branchData[l_idx]->addValue(BRANCH_METER,
        atoi(split_line[25].c_str()), nelems);

    // BRANCH_LENGTH        "LEN"                        float
    if (nstr > 26) p_branchData[l_idx]->addValue(BRANCH_LENGTH,
        atof(split_line[26].c_str()), nelems);

    // BRANCH_O1        "O1"                       integer
    if (nstr > 27) p_branchData[l_idx]->addValue(BRANCH_O1,
        atoi(split_line[27].c_str()), nelems);

    // BRANCH_F1        "F1"                             float
    if (nstr > 28) p_branchData[l_idx]->addValue(BRANCH_F1,
        atoi(split_line[28].c_str()), nelems);

    // BRANCH_O2        "O2"                       integer
    if (nstr > 29) p_branchData[l_idx]->addValue(BRANCH_O2,
        atoi(split_line[29].c_str()), nelems);

    // BRANCH_F2        "F2"                             float
    if (nstr > 30) p_branchData[l_idx]->addValue(BRANCH_F2,
        atoi(split_line[30].c_str()), nelems);

    // BRANCH_O3        "O3"                       integer
    if (nstr > 31) p_branchData[l_idx]->addValue(BRANCH_O3,
        atoi(split_line[31].c_str()), nelems);

    // BRANCH_F3        "F3"                             float
    if (nstr > 32) p_branchData[l_idx]->addValue(BRANCH_F3,
        atoi(split_line[32].c_str()), nelems);

    // BRANCH_O4        "O4"                       integer
    if (nstr > 33) p_branchData[l_idx]->addValue(BRANCH_O4,
        atoi(split_line[33].c_str()), nelems);

    // BRANCH_F4        "F4"                             float
    if (nstr > 34) p_branchData[l_idx]->addValue(BRANCH_F4,
        atoi(split_line[34].c_str()), nelems);
