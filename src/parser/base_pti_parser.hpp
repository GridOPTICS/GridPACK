...
class BasePTIParser : public BaseParser<_network>
{
  ...
  protected:
    ...
    void expandBusModels(void)
    {
      ...
      // Reset global indices start by assigning new global indices to local
      // buses. Do this in a way that will be independent of the number of
      // the number of processors used in the calculation
      for (i=0; i<nprocs; i++) {
        added_buses[i] = 0;
      }
      int newbuses = p_network->numBuses();
      for (i=0; i<newbuses; i++) {
        if (p_network->getActiveBus(i)) added_buses[me]++;
      }
      p_network->component()->sum(added_buses,nprocs);
      offset[0] = 0;
      for (i=1; i<nprocs; i++) {
      ...
      for (i=0; i<nbus; i++) {
        if (p_network->getActiveBus(i)) {
          if (i == comp_buses[icnt]) {
            ...
            p_network->setGlobalBusIndex(i,offset[me]+lcnt);
            lcnt++;
          }
        }
      }

      // Add global indices to new local branches. Unlike buses,
      // new global indices for branches depend on the number of processors
      ...
      for (i=0; i<nbranch; i++) {
        if (p_network->getActiveBranch(i)) branch_buf[me]++;
      }
      ...
      for (i=0; i<nbranch; i++) {
        if (p_network->getActiveBranch(i)) {
          p_network->setGlobalBranchIndex(i,icnt);
          icnt++;
        }
      }
