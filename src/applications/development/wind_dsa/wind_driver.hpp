...
class DataStore
{
   ...
   * @param nsteps number of timesteps being stored
   */
  DataStore(int nwatch, int nconf, int nsteps);
...
// Data structure for collecting time series data for a single simulation
// from each process and streaming it out all at once at the end of the
// simulation
class SimulationStore
{
   ...
   */
  SimulationStore(int nwatch, int nsteps, gridpack::parallel::Communicator &comm);
   ...
   * @param vals vector of time series values for a generator
   */
  void saveData(int gen_idx, std::vector<double> &vals);
   ...
   */
  void writeData(std::string &topic);
