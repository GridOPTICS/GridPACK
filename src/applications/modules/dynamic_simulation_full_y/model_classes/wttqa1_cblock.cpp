    ...
    data, int idx)
{
    ...
    delayblock_dly1.setparams(1, Ts1, Min1, Max1, -1000, 1000);
    delayblock_dly2.setparams(1, Ts2, Min1, Max1, -1000, 1000);
    piblock_pi.setparams(kp1, ki1, Min1, Max1, -1000, 1000);
}
...
void gridpack::dynamic_simulation::Wttqa1Model::init(double mag, double ang, double ts)
{
    ...
    double u1;
    u1 = piblock_pi.init_given_y(Pref);
}
...
void gridpack::dynamic_simulation::Wttqa1Model::predictor(double t_inc, bool flag)
{
    ...
    double u1, y1;
    u1 = Pg; // PSLF's Pg is PSSE's Pelec
    y1 = delayblock_dly1.getoutput(u1, t_inc, PREDICTOR, true);
    double u2, y2;
    u2 = y1;
    y2 = psblock.getoutput(u2);
    double u3, y3;
    u3 = y2;
    y3 = delayblock_dly1.getoutput(u3, t_inc, PREDICTOR, true);
    double u4, y4;
    if (Tflag == 1) 
	u4 = (Pref0 - y1) / wt;
    else 	
	u4 = wt - y3;
    y4 = piblock_pi.getoutput(u4, t_inc, PREDICTOR, true);
    Pref = y4 * wt;
...
void gridpack::dynamic_simulation::Wttqa1Model::corrector(double t_inc, bool flag)
{
    ...
    double u1, y1;
    u1 = Pg; // PSLF's Pg is PSSE's Pelec
    y1 = delayblock_dly1.getoutput(u1, t_inc, CORRECTOR, true);
    double u2, y2;
    u2 = y1;
    y2 = psblock.getoutput(u2);
    double u3, y3;
    u3 = y2;
    y3 = delayblock_dly1.getoutput(u3, t_inc, CORRECTOR, true);
    double u4, y4;
    if (Tflag == 1) 
	u4 = (Pref0 - y1) / wt;
    else 	
	u4 = wt - y3;
    y4 = piblock_pi.getoutput(u4, t_inc, CORRECTOR, true);
    Pref = y4 * wt;
}
