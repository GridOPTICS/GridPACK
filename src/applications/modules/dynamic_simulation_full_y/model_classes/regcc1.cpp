 ...
 * 
 * @brief  WECC generic generator/converter REGCC1 model
 * 
 * 
 */
 ...
 *  Basic constructor
 */
gridpack::dynamic_simulation::Regcc1Generator::Regcc1Generator(void)
{
  domega_g = 0.0;
  Thetapitch = 0.0;
  omega_ref = 1.0;
 ...
 */
void gridpack::dynamic_simulation::Regcc1Generator::load(
    ...
    data, int idx)
{
  ...
  if (!data->getValue(GENERATOR_REGC_KIP,  &Kip, idx)) Kip = 1;
  if (!data->getValue(GENERATOR_REGC_KII,  &Kii, idx)) Kii = 10;
  if (!data->getValue(GENERATOR_REGC_KPPLL,  &Kppll, idx)) Kppll = 1;
  if (!data->getValue(GENERATOR_REGC_KIPLL,  &Kipll, idx)) Kipll = 10;
  if (!data->getValue(GENERATOR_REGC_WMAX,  &wmax, idx)) wmax = 12;
  if (!data->getValue(GENERATOR_REGC_WMIN,  &wmin, idx)) wmin = -12;
  ...
  Ip_pi_blk.setparams(Kip,Kii);
  Iq_pi_blk.setparams(Kip,Kii);

  Omega_PLL_blk.setparams(Kppll,Kipll,-1000.0,1000.0,wmin,wmax);
 ...
 */
void gridpack::dynamic_simulation::Regcc1Generator::init(double Vm,
    double Va, double ts)
{
  theta = Va;
  ...
  Ip_pi_blk.init_given_y(Ip);
  
  Iqcmd = Iq_blk.init_given_y(Iq);

  Iq_pi_blk.init_given_y(Iq);
  ...
  // Initial angle for PLL
  delta = theta;
  Angle_blk.init_given_y(delta);

  Omega_PLL_blk.init_given_u(Vq);
 ...
 */
gridpack::ComplexType gridpack::dynamic_simulation::Regcc1Generator::INorton()
{
  ...
  transform(Ipout,Iqout,delta,&Irout,&Iiout);
 ...
 */
gridpack::ComplexType gridpack::dynamic_simulation::Regcc1Generator::NortonImpedence()
{
  double B = 0.0;
  double G = 0.0;
 ...
 */
void gridpack::dynamic_simulation::Regcc1Generator::predictor_currentInjection(bool flag)
{
  p_INorton = INorton();
}

void gridpack::dynamic_simulation::Regcc1Generator::computeModel(double t_inc, IntegrationStage int_flag,bool flag)
{
  double Pg,Qg;
  double Qext;
  ...
  Ip = Ip_pi_blk.getoutput(Ip-Ipout,t_inc,int_flag,true);
				   
  Iq = Iq_blk.getoutput(Iqcmd,t_inc,int_flag,true);

  Iq = Iq_pi_blk.getoutput(Iq-Iqout,t_inc,int_flag,true);

  gridpack::ComplexType V = gridpack::ComplexType(Vt,Vq);
  ...
  dw = Omega_PLL_blk.getoutput(Vq,t_inc,int_flag,true);

  delta = Angle_blk.getoutput(dw,t_inc,int_flag,true);
 ...
 * @param flag initial step if true
 */
void gridpack::dynamic_simulation::Regcc1Generator::predictor(
 ...
 * @param flag initial step if true
 */
void gridpack::dynamic_simulation::Regcc1Generator::corrector_currentInjection(bool flag)
{
  p_INorton = INorton();
}
 ...
 * @param flag initial step if true
 */
void gridpack::dynamic_simulation::Regcc1Generator::corrector(
    double t_inc, bool flag)
{
  computeModel(t_inc,CORRECTOR,flag);
}

bool gridpack::dynamic_simulation::Regcc1Generator::tripGenerator()
{
	return false;
}
...
* 
*/
bool gridpack::dynamic_simulation::Regcc1Generator::applyGeneratorParAdjustment(int controlType, double newParValScaletoOrg){
 ...
 */
void gridpack::dynamic_simulation::Regcc1Generator::setVoltage(
    gridpack::ComplexType voltage)
{
  ...
  // Transformation such that Vm aligns with VR
  // Rotation by -delta
  transform(VR,VI,-delta,&Vt,&Vq);
}

/**
 * Set frequency on each generator, frequency is perunit
 */
void gridpack::dynamic_simulation::Regcc1Generator::setFreq(double dFreq)
{
   busfreq = dFreq;
}
 ...
 */
bool gridpack::dynamic_simulation::Regcc1Generator::serialWrite(
 ...
 */
void gridpack::dynamic_simulation::Regcc1Generator::getWatchValues(
 ...
 */   
bool gridpack::dynamic_simulation::Regcc1Generator::setState(std::string name,
    double value)
{
  return false;
}
 ...
 * @return false if no variable corresponding to name is found
 */   
bool gridpack::dynamic_simulation::Regcc1Generator::getState(std::string name,
    double *value)
{
  return false;
}
