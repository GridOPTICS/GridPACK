...
class VSAppModule
{
  public:
     ...
     */
    void readNetwork(boost::shared_ptr<VSNetwork> &network,
	...
	*/
	...
	bool getVSSolutionSingleBus(int bus_number, double &bus_mag, double &bus_angle);
     ...
     */
    bool setContingency(VSContingency &event);
     ...
     * @return false if location of contingency is not found in network
     */
    bool unSetContingency(VSContingency &event);
  ...
  private:
    ...
    boost::shared_ptr<gridpack::serial_io::SerialBusIO<VSNetwork> > p_busIO;

    // pointer to branch IO module
    boost::shared_ptr<gridpack::serial_io::SerialBranchIO<VSNetwork> > p_branchIO;
