 ...
 */
void gridpack::voltage_stability::VSFactoryModule::setYBus(void)
{
  ...
  for (i=0; i<numBranch; i++) {
    dynamic_cast<VSBranch*>(p_network->getBranch(i).get())->setYBus();
  }

  // Invoke setYBus method on all bus objects
  for (i=0; i<numBus; i++) {
    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setYBus();
  }
...
/**
  * Make SBus vector 
  */
void gridpack::voltage_stability::VSFactoryModule::setSBus(void)
{
  int numBus = p_network->numBuses();
  int i;
  ...
  for (i=0; i<numBus; i++) {
    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setSBus();
  }
}
...
/**
  * Create the PQ 
  */
void gridpack::voltage_stability::VSFactoryModule::setPQ(void)
{
  int numBus = p_network->numBuses();
  int i;
  ...
  for (i=0; i<numBus; i++) {
    dynamic_cast<VSBus*>(p_network->getBus(i).get())->vectorValues(values);
  }
}
 ...
 * @param value
 */
//void gridpack::voltage_stability::VSFactoryModule::updatePg(std::string &name, int busID, std::string genID, double value)
void gridpack::voltage_stability::VSFactoryModule::updatePg(int busID, std::string genID, double value)
{
  int numBus = p_network->numBuses();
  int i;
  int genIndex=0;
  for (i=0; i<numBus; i++) {
//    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setParam(name,busID, genID, value);
    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setParam(GENERATOR_PG,
        busID, genID, value);
  }
}

void gridpack::voltage_stability::VSFactoryModule::updateQg(int busID, std::string genID, double value)
{
  int numBus = p_network->numBuses();
  int i;
  int genIndex=0;
  for (i=0; i<numBus; i++) {
//    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setParam(name,busID, genID, value);
    dynamic_cast<VSBus*>(p_network->getBus(i).get())->setParam(GENERATOR_QG,
        busID, genID, value);
  }
}
 ...
 */
bool gridpack::voltage_stability::VSFactoryModule::checkLoneBus(std::ofstream *stream)
{
  int numBus = p_network->numBuses();
  int i, j, k;
  bool bus_ok = true;
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::clearLoneBus()
{
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::setVoltageLimits(double Vmin,
    double Vmax)
{
  int numBus = p_network->numBuses();
  int i;
  ...
  for (i=0; i<numBus; i++) {
    gridpack::voltage_stability::VSBus *bus =
      dynamic_cast<gridpack::voltage_stability::VSBus*>(p_network->getBus(i).get());
 ...
 */
bool gridpack::voltage_stability::VSFactoryModule::checkVoltageViolations()
{
  int numBus = p_network->numBuses();
  int i;
  bool bus_ok = true;
    ...
    if (p_network->getActiveBus(i)) {
      ...
      if (!bus->getIgnore() && bus->getArea() == area) {
        if (!bus->checkVoltageViolation()) {
          bus_ok = false;
          gridpack::voltage_stability::VSFactoryModule::Violation violation;
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::ignoreVoltageViolations()
{
  int numBus = p_network->numBuses();
  int i;
  for (i=0; i<numBus; i++) {
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::clearVoltageViolations()
{
  int numBus = p_network->numBuses();
  int i;
  for (i=0; i<numBus; i++) {
 ...
 * @return true if no violations found
 */
bool gridpack::voltage_stability::VSFactoryModule::checkLineOverloadViolations()
{
  int numBranch = p_network->numBranches();
  int i;
  bool branch_ok = true;
 ...
 * @return true if no violations found
 */
bool gridpack::voltage_stability::VSFactoryModule::checkLineOverloadViolations(int area)
{
  int numBranch = p_network->numBranches();
  int i;
  bool branch_ok = true;
    ...
    std::vector<std::string> &tags, std::vector<bool> &violations)
{
  ...
  for (i=0; i<nbranch; i++) {
    ...
    for (j=0; j<indices.size(); j++) {
      gridpack::voltage_stability::VSBranch *branch = p_network->getBranch(indices[j]).get();
      // Loop over all lines in the branch and choose the smallest rating value
      int nlines;
 ...
 * checks are not counted as violations
 */
void gridpack::voltage_stability::VSFactoryModule::ignoreLineOverloadViolations()
{
  int numBranch = p_network->numBranches();
  int i;
  for (i=0; i<numBranch; i++) {
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::clearLineOverloadViolations()
{
  int numBranch = p_network->numBranches();
  int i;
  for (i=0; i<numBranch; i++) {
 ...
 * @return true if no violations found
 */
bool gridpack::voltage_stability::VSFactoryModule::checkQlimViolations()
{
  int numBus = p_network->numBuses();
  int i;
  bool bus_ok = true;
 ...
 * @return true if no violations found
 */
bool gridpack::voltage_stability::VSFactoryModule::checkQlimViolations(int area)
{
  int numBus = p_network->numBuses();
  int i;
  bool bus_ok = true;
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::clearQlimViolations()
{
  int numBus = p_network->numBuses();
  int i;
  bool bus_ok = true;
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::resetVoltages()
{
  int numBus = p_network->numBuses();
  int i;
  for (i=0; i<numBus; i++) {
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::scaleGeneratorRealPower(
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::scaleLoadPower(
    double scale, int area, int zone)
{
  int nbus = p_network->numBuses();
  int i, izone;
 ...
 * @return total load
 */
double gridpack::voltage_stability::VSFactoryModule::getTotalLoadRealPower(int area, int zone)
{
  double ret = 0.0;
  int nbus = p_network->numBuses();
  int i, j, izone;
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::getGeneratorMargins(int area, int zone,
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::resetPower()
{
  int nbus = p_network->numBuses();
  int i;
  for (i=0; i<nbus; i++) {
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::setRTPRParams(
 ...
 */
std::vector<gridpack::voltage_stability::VSFactoryModule::Violation>
gridpack::voltage_stability::VSFactoryModule::getViolations()
{
  std::vector<gridpack::voltage_stability::VSFactoryModule::Violation> ret;
  gridpack::parallel::GlobalVector<gridpack::voltage_stability::VSFactoryModule::Violation>
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::clearViolations()
{
  p_violations.clear();
}
 ...
 */
void gridpack::voltage_stability::VSFactoryModule::useRateB(bool flag)
{
  if (flag) {
    p_rateB = true;
  } else {
