 ...
 * @file   gast.hpp
 *
 * @brief GAST governor model header file  
 * 
 * 
 */
...
class Gast: public BaseEMTGovModel
{
public:
  /**
   * Basic constructor
...
private:
  ...
  double R, T1, T2, T3, AT, KT, Vmax, Vmin, Dt;
  ...
  // Gast blocks
  Filter delay_blk_T1;    // Delay block with time constant T1
  double  delay_blk_T1_out;  // Output of delay block T1

  Filter delay_blk_T2;    // Delay block with time constant T2
  double  delay_blk_T2_out;  // Output of delay block T2
  
  Filter delay_blk_T3;    // Delay block with time constant T3
  double  delay_blk_T3_out;  // Output of delay block T3

  // GAST state variables
  double x1;  // Fuel valve
  double x2;  // Fuel flow
  double x3;  // Exhaust temperature

  // GAST output
  double xout;
  
  // GAST state-variable derivatives
  double dx1, dx2, dx3;
  ...
  // Flag for adjusting limiter values during initialization
...
public:
  ...
  // Default constructor
  // Yuan: Event(2) here means there are 2 events
  GastEvent(Gast *gov):gridpack::math::RealDAESolver::Event(2),p_gov(gov)
  {
    std:fill(p_term.begin(),p_term.end(),false);
	// Yuan: detect crosszeros: pos->neg (CrossZeroNegative), neg->pos (CrossZeroPositive), or both
	// CrossZeroNegative: detect and trigger an event when evalues[idx] changes from pos. to neg.
