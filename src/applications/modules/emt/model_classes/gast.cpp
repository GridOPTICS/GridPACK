...
void Gast::load(const boost::shared_ptr<gridpack::component::DataCollection> data, int idx)
{
  ...
  printf("R=%f,T1=%f,T2=%f,T3=%f,AT=%f,KT=%f,Vmax=%f,Vmin=%f,Dt=%f\n",R,T1,T2,T3,AT,KT,Vmax,Vmin,Dt);
...
void Gast::init(gridpack::RealType* xin) 
{
  ...
  if(integrationtype != IMPLICIT) {
    ...
    if (OptionToModifyLimitsForInitialStateLimitViolation) {
      if (delay_blk_T1_out > Vmax) Vmax = delay_blk_T1_out + 0.1;
      if (delay_blk_T1_out < Vmin) Vmin = delay_blk_T1_out - 0.1;
    }
